...
@section('content')
    <div class="container">
        ...
        @if ($ticket->status === 'open')
            <form action="{{ route('artist.support.respond', $ticket->id) }}" method="POST" class="mb-3">
                @csrf
                <div class="mb-3">
        ...
        @if ($ticket->status === 'open')
            <form id="close-ticket-form" action="{{ route('artist.support.close', $ticket->id) }}" method="POST">
                @csrf
                <button type="submit" id="close-ticket-button" class="btn btn-danger">Close Ticket</button>
            </form>
        @endif

        <a href="{{ route('artist.support.index') }}" class="btn btn-secondary mt-3">Back to Support Tickets</a>
    </div>
@endsection
@section('scripts')
